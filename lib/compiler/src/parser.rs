...
fn n(n: i64) -> SExp { SExp::Number(n) }
fn i(x: &'static str) -> SExp { SExp::Ident(x.to_string()) }
fn k(x: &'static str) -> SExp { SExp::Keyword(x.to_string()) }
fn q(x: SExp) -> SExp { SExp::Quote(Box::new(x)) }
fn p(xs: Vec<SExp>) -> SExp { SExp::List(Bracket::Paren, xs) }
fn s(xs: Vec<SExp>) -> SExp { SExp::List(Bracket::Square, xs) }
fn c(xs: Vec<SExp>) -> SExp { SExp::List(Bracket::Curly, xs) }
...
peg::parser!{
    grammar sexp_parser() for str {
        ...
        pub rule sexp() -> SExp
            = n:num() _ { SExp::Number(n) }
            / x:ident() _ { SExp::Ident(x) }
            / ['\''] x:sexp() _ { SExp::Quote(Box::new(x)) }
            / [':'] x:ident() _ { SExp::Keyword(x) }
            / xs:bracket(Paren) _ { SExp::List(Paren, xs) }
            / xs:bracket(Square) _ { SExp::List(Square, xs) }
            / xs:bracket(Curly) _ { SExp::List(Curly, xs) }
...
mod tests {
    ...
    fn test_lists() {
        ...
        assert_eq!(sexp("([] {} ())").unwrap(), p(vec![s(vec![]), c(vec![]), p(vec![])]));

        assert_eq!(sexp("(if (= 1 2) (print [1 2 3]) (error {1 2}))").unwrap(),
                   p(vec![i("if"), p(vec![i("="), n(1), n(2)]),
